...
document.addEventListener("DOMContentLoaded", function(){
    ...
    window.handleImageError = function(img) {
        ...
        img.src = `./blogposts/${entryId}/images/cardimage.${IMAGE_EXTENSIONS[extIndex]}`;
    };
    ...
    async function loadData() {
        jsonentries = await fetch("./blogposts/posts.json");
    ...
    function createCard(entry) {
        ...
        card.href = `./blogposts/${entry.id}/post.html`;
        const formattedDate = formatDate(entry.date);

        card.innerHTML = `
        <div>
        <img src="./blogposts/${entry.id}/images/cardimage.${IMAGE_EXTENSIONS[0]}"
             alt="${entry.title}"
             class="cardimage"
        ...
        <h3 class="blogposttitle">${entry.title}</h3>
        <div class="description">${entry.description}</div>
        <div class="blogpostdate">${formattedDate}</div>
        </div>
        `;
